...
func GetServiceLogs(ctx context.Context, deviceConn net.Conn, applicationID, service string, metricPath string, metricPort uint) (*http.Response, error) {
	serviceURL := url.URL{
		Path: fmt.Sprintf(
			"/applications/%s/services/%s/logs",
			applicationID, service,
		),
	}
